...
Definition eps_rel   (A: J)   := {s: A ⨉ A| ∃ x, ∃ y, s = ⟨x, y⟩ ∧ x ∈ y}.
...
Lemma eps_self_id: ∀ A, trans A → wo (ER(A)) A → E(ER(A), A) = id A.
  intros A P1 P2.
...
Lemma eps_image_self: ∀ A, trans A → wo (ER(A)) A → EI(ER(A), A) = A.
Proof.
  intros A P1 P2.
  pose (eps_self_id _ P1 P2) as P3.
  unfold eps_image.
  apply (eq_cr (λ x, ran(x) = A) P3).
  apply id_ran.
Qed.

Lemma eps_self: ∀ A, ∀ x, trans A → wo (ER(A)) A → x ∈ A → E(ER(A), A)[x] = x.
Proof.
  intros A x P1 P2 P3.
  pose (eps_self_id _ P1 P2) as P4.
  apply (eq_cr (λ s, s[x] = x) P4).
  apply (id_fval _ _ P3).
Qed.
...
Lemma in_eps_image: ∀ R, ∀ A, ∀ x, wo R A → x ∈ EI(R, A) → x = EI(ER(x), x).
Proof.
  intros R A x P1 P2.
  pose (wo_eps_image _ _ P1) as P3.
  pose (eps_image_t _ _ P1) as P4.
  apply eq_s.
  apply eps_image_self.
  + destruct (eps_image_e _ _ _ P1 P2) as [s [Q1 Q2]].
    apply (eq_cr (λ x, trans x) Q2).
    apply (eps_trans _ _ _ P1 Q1).
  + pose (sub_wo _ _ _ P3 (trans_e2 _ P4 _ P2)) as Q1.
    pose (eps_rel_eq _ _ (trans_e2 _ P4 _ P2)) as Q2.
