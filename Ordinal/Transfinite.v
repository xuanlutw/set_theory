...
(*Lemma eps_e1: forall R A t, wo R A -> t ∈ A -> *)
...
(*Lemma eps_e2: forall R A x s, wo R A -> x ∈ A -> s ∈ (eps R A)[x] ->*)
  (*exists y, s = (eps R A)[y] /\ y ∈ A /\ y <[R] x.*)
(*Proof.*)
  (*intros R A x s P1 P2 P3.*)
  (*rewrite (eps_e1 _ _ _ P1 P2) in P3.*)
  (*destruct (image_e _ _ _ P3) as [y [P4 P5]].*)
  (*exists y.*)
  (*split.*)
  (*+ apply (fval_i).*)
...
(*Proof.*)
  (*intros R A t P1 P2.*)
  (*pose (sub_ctor (fun x => (eps R A)[x] ∈ (eps R A)[x]) A) as S.*)
  (*destruct (LEM (S = ∅)) as [P3|P3].*)
  (*+ apply (sub_empty _ _ _ P3 P2).*)
  (*+ destruct (wo_least_elmn _ _ P1 _ (sub_e2 _ _) P3) as [x [Q1 Q2]].*)
    (*destruct (sub_e _ _ _ Q1) as [Q3 Q4].*)
    (*assert (eps R A [x] ∈ eps R A [|seg R A x|]) as Q5.*)
    (*{ rewrite <- (eps_e1 R A x P1 Q3).*)
      (*apply Q4. }*)
    ...
    (*- apply (wo_le _ _ _ _ P1 Q3 Q8).*)
      (*apply Q2.*)
      (*apply (sub_i _ _ _ Q8).*)
      (*rewrite <- (fval_i _ _ _ (eps_function _ _ P1) Q6).*)
      (*apply Q4.*)
    (*- apply Q9.*)
(*Qed.*)
...
(*Proof.*)
  ...
  (*+ intros a1 a2 b P2 P3.*)
    ...
    (*- pose (eps_less _ _ _ _ P1 P4 P5 P6) as Q1.*)
      (*rewrite <- (fval_i _ _ _ (eps_function _ _ P1) P2) in Q1.*)
      (*rewrite (fval_i _ _ _ (eps_function _ _ P1) P3) in Q1.*)
...
(*Lemma eps_image_i1: forall R A x, wo R A -> x ∈ A ->*)
...
(*Lemma eps_image_i2: forall R A x s, wo R A -> x ∈ A -> s ∈ (eps R A)[x] ->*)
  (*s ∈ (eps_image R A).*)
(*Proof.*)
  (*intros R A x s P1 P2 P3.*)
...
(*Lemma eps_image_e: forall R A x, wo R A -> x ∈ (eps_image R A) -> *)
...
(*Proof.*)
  ...
  (*+ rewrite <- (eps_dom _ _ P1).*)
    (*apply (dom_i2 _ _ _ P3).*)
  (*+ apply (fval_i _ _ _ (eps_function _ _ P1) P3).*)
(*Qed.*)
...
(*Proof.*)
  ...
  (*rewrite P5 in P2.*)
  (*apply (eps_image_i2 _ _ _ _ P1 P4 P2).*)
(*Qed.*)

(*Lemma eps_rel_i: forall R A a1 a2, wo R A -> a1 ∈ A -> a2 ∈ A -> *)
...
(*Lemma eps_rel_e: forall R A a1 a2, wo R A -> a1 ∈ A -> a2 ∈ A -> *)
  (*(eps R A)[a1] <[eps_rel R A] (eps R A)[a2] -> (eps R A)[a1] ∈ (eps R A)[a2].*)
(*Proof.*)
  (*intros R A a1 a2 P1 P2 P3 P4.*)
  (*destruct (sub_e _ _ _ P4) as [_ [x [y [P5 P6]]]].*)
  (*rewrite (opair_equal_efst _ _ _ _ P5).*)
  (*rewrite (opair_equal_esnd _ _ _ _ P5).*)
  (*apply P6.*)
(*Qed.*)
